...
function GetMillWiseProductionReportList(jsonData) {
    ...
    MillWiseProductionReport.GetItemSizeWiseAndMonthWiseData(jsonData).then(function (response) {
        if (response && response.length > 0) {
            ...
            RenderMillWiseProductionTable(response, currentIsMachineWise);
            $('#btnDownload').show();
...
function RenderMillWiseProductionTable(data, isMachineWise) {
    ...
    if (isMachineWise) {
        ...
        headerHtml += '<th rowspan="2" style="text-align:center; vertical-align:middle; background-color:#e3f2fd;">Total OD Wise</th>';
        ...
        headerHtml += '<th style="text-align:center; background-color:#e3f2fd;">Total OD Wise</th>';
    ...
    data.forEach(function(row) {
        ...
        if (numericColumn) {
            ...
            if (sizeKey && !sizeFirstOccurrence[sizeKey]) {
                ...
                bodyHtml += '<td rowspan="' + rowCount + '" style="text-align:right; font-weight:bold; vertical-align:middle; background-color:#e3f2fd;">' + sizeWiseVal + '</td>';
            } else if (!sizeKey) {
    ...
    bodyHtml += '<td style="text-align:right; background-color:#e3f2fd;"></td>';
...
// Legacy single-size filter kept for backward compatibility; now column-specific
...
window.applySizeFilter = function() {
    ...
    toastr.success('Filter applied: ' + activeSizeFilters.length + ' size(s) selected');
};
...
window.clearSizeFilter = function() {
    ...
    toastr.info('Filter cleared - showing all sizes');
};
...
window.applyColumnFilter = function(columnName) {
    ...
    toastr.success('Filter applied on ' + col);
};
...
window.clearColumnFilter = function(columnName) {
    ...
    RenderMillWiseProductionTable(filteredReportData, currentIsMachineWise);
    $('.col-filter-dropdown[data-col="' + col + '"]').hide();
    toastr.info('Filter cleared on ' + col);
};
