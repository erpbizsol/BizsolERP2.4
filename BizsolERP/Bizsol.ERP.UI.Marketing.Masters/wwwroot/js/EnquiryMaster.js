...
$(document).ready(function () {
    ...
    // Numeric-only filters for grid inputs
    // Allow only numbers and a single decimal point for quantity
    $(document).on('input', '.pd-qty', function () {
        let val = this.value || '';
    ...
    // Allow only digits for contact mobile number
    $(document).on('input', '.pd-ContactMobileNo', function () {
        this.value = (this.value || '').replace(/[^0-9]/g, '');
    });
    ...
    // Ensure Enquiry Date defaults to today on initial load if blank
    try { SetTodayEnquiryDate(); } catch (e) {}
    // Ensure vehicle suggestion list container exists (for custom autosuggestions)
    try {
...
$('#ddlCompanyName, #ddlEnquiryType, #ddlCountry, #ddlState, #ddlCity, #ddlNextFollowupMode, #txtEnquiryDate, #ddlLeadSource, #txtReferenceDate, #ddlAssignSalesman, #txtNextFollowupDate')
    .change(function () {
        if ($(this).val() != '') {
...
function GetLeadMasterList(SalesPerson) {
    LeadMasterService.GetLeadMasterList(SalesPerson).then(function (response) {
        ...
        if (response.length > 0) {
            ...
            const Button = false;
            const showButtons = [];
            const StringdoubleFilterColumn = ["City"];
            const hiddenColumns = ["Lead Date", "Next Followup Date","Code", "ReferenceNo", "ReferenceDate", "PinCode", "CustomerFromMaster", "AccountContactPersonDetail", "UserID", "MarketingPersonMaster_Code", "Address1", "Address2", "Nation", "PhoneNo", "MobileNo", "FaxNo", "EMail", "Remark", "FinYear", "DeliveryDays", "VerifiedBy", "UserVerifiedBy", "VerifiedOn", "DeliveryRemark", "Specification", "CustomerType", "EnquiryType_Code", "EnquiryTypeName", "SortOrder", "NextFollowupMode", "LeadSourceMaster_Code", "LeadSourceDespName", "ReferenceBy", "Website", "CurrencyMaster_Code", "Currency", "ConversionRate", "FreightPerKG", "ContactPersonFromMaster", "ContactPersonName", "TestingGroupMaster_Code", "TestingGroup", "EnquiryToVendor", "EnquiryToVendorVerify", "PriceToVendorRemark", "ReasonForReject"]
            const ColumnAlignment = {
                Action: ";width:150px;"
            };
            ...
            const updatedResponse = response.map(item => {
                let buttonsHTML = `<button class="btn btn-warning icon-height mb-1" title="Edit"  onclick="GetEnquiryDetailsByCode(${item.Code},this)"><i class="fa fa-pencil"></i></button>
                ...
                <ul class="dropdown-menu p-1">
                  ...
                  <li style="padding:1px;"><input type="button" style="width:100%;height: 33px;" value="Assign" class="btn btn-info mb-1" title="Assign"  onclick="AssignEnquiry(${item.Code})">
...
function CreateNew() {
    ...
    // Set Enquiry Date to today when starting new
...
async function GetEnquiryDetailsByCode(Code) {
    ...
    try {
        ...
        if (resObj.EnquiryDetails.length > 0) {
            ...
            let grid = "";
            $.each(EnquiryDetails, function (i, product) {
                grid += "<tr>";
                grid += "<td><select id='txtItemName_" + product.Code + "' class='form-control form-control-sm pd-name' data-current-value='" + (product["Product Name"] || '') + "'></select></td>";
                grid += "<td><input  type='text' id='txtSpecification_" + product.Code + "'  class='form-control form-control-sm pd-spec' value='" + (product["Specification"] || '') + "' maxlength='100' autocomplete='off' /></td>";
                grid += "<td><select id='txtUOM_" + product.Code + "' class='form-control form-control-sm pd-uom' data-current-value='" + (product["UOM"] || '') + "'></select></td>";
                grid += "<td><input type='text' id='txtQuantity_" + product.Code + "' class='form-control form-control-sm pd-qty' value='" + (product["Quantity"] || '') + "' maxlength='10' autocomplete='off' /></td>";
                grid += "<td><input type='text' id='txtProductRemarks_" + product.Code + "' class='form-control form-control-sm pd-remark' value='" + (product["Remarks"] || '') + "' maxlength='150' autocomplete='off' /></td>";
                grid += "<td class='text-center'><button type='button' class='btn btn-sm btn-info' onclick='ChangeProduct(" + product.Code + ")'><i class='fa fa-save'></i></button>&nbsp;<button type='button' class='btn btn-sm btn-danger' onclick='DeleteProductDetail(" + product.Code + ")'><i class='fa fa-times'></i></button></td>";
                grid += "</tr>";
            });
        ...
        if (resObj.ContactPersonsList.length > 0) {
            ...
            $.each(resObj.ContactPersonsList, function (i, person) {
                grid += "<tr>";
                grid += "<td><input type='text' id='txtName_"+person.Code+"'class='form-control form-control-sm cp-name' value='" + (person.Name || '') + "' maxlength='50' autocomplete='off' /></td>";
                grid += "<td><input type='text' id='txtDepartment_" + person.Code +"' class='form-control form-control-sm cp-dept' value='" + (person.Department || '') + "' autocomplete='off' /></td>";
                grid += "<td><input type='text' id='txtDesignation_" + person.Code +"' class='form-control form-control-sm cp-desig' value='" + (person.Designation || '') + "' autocomplete='off' /></td>";
                grid += "<td><input type='email'id='txtContactEmail_" + person.Code +"' class='form-control form-control-sm cp-email' value='" + (person['Email Id'] || '') + "' maxlength='100' autocomplete='off' /></td>";
                grid += "<td><input type='text' id='txtContactMobileNo_" + person.Code +"'class='form-control form-control-sm cp-mobile' value='" + (person['Contact No'] || '') + "' maxlength='10' autocomplete='off' /></td>";
                grid += "<td class='text-center'><button type='button' class='btn btn-sm btn-info' onclick='ChangeContact(" + person.Code + ")'><i class='fa fa-save'></i></button>&nbsp;<button type='button' class='btn btn-sm btn-danger' onclick='DeleteContactPerson(" + person.Code +")'><i class='fa fa-times'></i></button></td>";
                grid += "</tr>";
            });
...
function CreateContactNewRow() {
    ...
    grid += "<td><input type='text'id='txtContactMobileNo_0' class='form-control form-control-sm pd-ContactMobileNo' value='' maxlength='10' autocomplete='off' /></td>";
    grid += "<td class='text-center'><button type='button' class='btn btn-sm btn-info' onclick='ChangeContact(0)'><i class='fa fa-save'></i></button></td>";
    grid += "</tr>";
    return grid;
}
function CreateProductNewRow() {
    ...
    grid += "<td><input type='text'id='txtQuantity_0' class='form-control form-control-sm pd-qty' value='' autocomplete='off' maxlength='10'  /></td>";
    ...
    grid += "<td class='text-center'><button type='button' class='btn btn-sm btn-info' onclick='ChangeProduct(0)'><i class='fa fa-save'></i></button></td>";
    grid += "</tr>";
    return grid;
}
...
function GetDepartmentlist() {
    ...
    const deptPromise = LeadMasterService.GetDepartmentlist();
    ...
    Promise.all([deptPromise, desigPromise]).then(function (results) {
        ...
        const departments = deptRes
            .map(function (item) { return { Desp: item.DepartmentName || item['Department'] || '' }; })
            .filter(function (x) { return x.Desp; });
        const designations = desigRes
            .map(function (item) { return { Desp: item.DesignationName || item['Designation'] || '' }; })
            .filter(function (x) { return x.Desp; });
