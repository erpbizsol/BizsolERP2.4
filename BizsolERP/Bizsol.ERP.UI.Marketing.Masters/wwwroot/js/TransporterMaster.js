...
let G_ModuleName = 'Transporter Master';
window.G_VendorHasVerifyRight = false;
window.G_PartyVerificationBeforeOrderY = false;

function syncTransporterModuleContextFromHeading() {
    var raw = ($('#ERPHeading').text() || '').trim();
    G_ModuleName = raw || 'Transporter Master';
}
...
function applyTransporterPartyVerificationUi() {
    var show = !!window.G_PartyVerificationBeforeOrderY;
    var $strip = $('#transporterStatsStrip');
...
$(document).ready(function () {
    ...
    VendorMasterService.GetFixedParameterDetails()
        ...
        .finally(function () {
            applyTransporterPartyVerificationUi();
...
function mapVendorRowsToGrid(rows) {
    return rows.map(function (item) {
        var verifyCell = '';
        if (shouldShowTransporterPartyVerifyColumn()) {
        ...
        var patch = {};
        if (shouldShowTransporterPartyVerifyColumn()) patch.Verify = verifyCell;
...
function syncVendorStatChipClasses() {
    ...
    $('#transporterStatsStrip .vm-stat-chip[data-vm-filter]')
        .removeClass('vm-stat-chip--active')
        .attr('aria-pressed', 'false');
    $('#transporterStatsStrip .vm-stat-chip[data-vm-filter="' + mode + '"]')
...
function GetTransporterMasterList() {
    TransporterMasterService.GetSolarTransporterMasterList('T')
        .then(function (response) {
            var rows = [];
            ...
            if (rows.length > 0) {
                ...
                toastr.warning('No transporters found.');
        ...
        .catch(function () {
            ...
            toastr.error('Failed to load transporter list.');
        });
}
...
function VerifyVendor(code) {
    ...
    $('#vmVerifyConfirmTitle').text('Verify this transporter?');
    $('#vmVerifyConfirmText').text('This will mark the transporter as verified.');
