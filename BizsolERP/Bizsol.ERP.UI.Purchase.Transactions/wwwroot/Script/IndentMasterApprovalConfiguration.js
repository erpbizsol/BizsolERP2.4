import { IndentMasterApprovalConfigurationService } from '../../Bizsol.WebERP.UI.Shared/js/JSServices/IndentMasterApprovalConfigurationService.js';
...
async function LoadGroupList() {
    try {
        const result = await IndentMasterApprovalConfigurationService.GetApproverGroupList();
        G_GroupList = _toList(result);
    } catch (e) {
        G_GroupList = [];
    }
    PopulateGroupSelect();
...
function PopulateGroupSelect() {
    ...
    G_GroupList.forEach(function (g) {
        ...
        opt.value = g.Code ?? g.code ?? g.GroupMaster_Code;
        opt.textContent = g.GroupName || g.groupName || g.Desp || String(opt.value);
        sel.appendChild(opt);
    });
    InitSelect2();
}
...
async function LoadLevelList() {
    try {
        const result = await IndentMasterApprovalConfigurationService.GetLevelList();
        G_LevelList = _toList(result);
    } catch (e) {
        G_LevelList = [];
    }
    RenderTable();
...
async function OpenEditForm(code) {
    ...
    Showloader();
    try {
        const result = await IndentMasterApprovalConfigurationService.GetLevelById(code);
        const item = _toList(result)[0];
        if (!item) {
        ...
        let groupCode = '';
        if (item.GroupMaster_Code != null && String(item.GroupMaster_Code) !== '0') {
            groupCode = String(item.GroupMaster_Code);
        } else {
            const matched = G_GroupList.find(function (g) { return g.GroupName === item.GroupName; });
            groupCode = matched ? String(matched.Code) : '';
        }
        try { $('#frmGroup').val(groupCode).trigger('change'); } catch (e) { }
...
async function SaveLevel() {
    ...
    let userCode = 0;
    try { userCode = JSON.parse(sessionStorage.getItem('authKey')).UserMaster_Code; } catch (e) { }
    ...
    Showloader();
    try {
        const result = await IndentMasterApprovalConfigurationService.SaveIndentMasterApprovalConfig(payload);
...
async function ConfirmDeleteLevel() {
    ...
    Showloader();
    try {
        const result = await IndentMasterApprovalConfigurationService.DeleteIndentMasterApprovalConfig(code, reason);
        if (result && result.Status === 'Y') {
