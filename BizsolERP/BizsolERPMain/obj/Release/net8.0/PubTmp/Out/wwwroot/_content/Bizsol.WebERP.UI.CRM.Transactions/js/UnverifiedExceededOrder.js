...
$(document).ready(function () {
    $("#ERPHeading").text("Verify Exceeded Discount Order/Visit");
...
function GetOrderVerifyData() {
    ...
    if (SalesPerson === '') {
        ...
        GetUnVerifiedExceededDiscountList(SalePerson.trim(), Dealer.trim(), OrdersType, ChkWithOrder);
    }
}
...
function GetUnVerifiedExceededDiscountList(SalesPerson, DealerName, OrderType, ChkWithOrder) {
    VisitOrderEntryService.UnVerifiedExceededDiscountList(SalesPerson, DealerName, OrderType, ChkWithOrder).then(function (response) {
        if (response.length > 0) {
            $("#txtTable").show();
            const StringFilterColumn = ["Order Id", "Sale Person", "Visit Type"];
            const NumericFilterColumn = ["Over Due Amount", "Out Standing", "Total Final Amount", "Discount", "Basic Rate", "Final Rate"];
            const DateFilterColumn = ["Visit Date"];
            const Button = false;
            const showButtons = [];
            const StringdoubleFilterColumn = ["Zone", "Dealer Name", "City", "State"];
            const hiddenColumns = ["Code", "Total Order Amount", "Avg Rate", "OrderVisitType", "VerifiedLv1","VerifiedLv2"];
