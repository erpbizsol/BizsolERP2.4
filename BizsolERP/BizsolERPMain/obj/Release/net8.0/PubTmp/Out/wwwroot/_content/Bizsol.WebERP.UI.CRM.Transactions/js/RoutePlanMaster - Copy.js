 ...
 function GetNestedDealerList() {
    RoutePlanMasterService.GetNestedDealerList().then(function (response) {
       ...
       if (response.length > 0) {
                       ...
                       BindSelectList($('#ddldealerName1')[0], response.map((item) => ({ Code: item.Code, Desp: item.AccountDesp })), 'FirstItemZero');
                       $('#ddldealerName1').select2({
                           allowClear: true,
                           width: '300px' ,
...
function RoutePlanVistTypeDropDownList(){
    RoutePlanMasterService.RoutePlanVistTypeDropDownList().then(function (response) {
            ...
            if (response.length > 0) {
                ...
                BindSelectList($('#ddlVisitType1')[0], response.map((item) => ({ Code: item.Code, Desp: item.Desp })), 'FirstItemSelected');
                $('#ddlVisitType1').select2({
                    allowClear: true,
                    matcher: function (params, data) {
                        // If there's no search term, return all data
                        if ($.trim(params.term) === '') {
                ...
                getVisitType('ddlVisitType1','ddldealerName1',1);
 ...
 function GetCityList(CountryName,StateName) {
    RoutePlanMasterService.GetCityList(CountryName,StateName).then(function (response) {
        ...
        if (response.length > 0) {
            ...
            BindSelectList($('#ddlCity1')[0], response.map((item) => ({ Code: item.Code, Desp: item.CityName })), 'FirstItemZero');
            $('#ddlCity1').select2({
                allowClear: true,
                width: '200px',
                matcher: function (params, data) {
...
function GetCityDetailsByName(x) {
    ...
    //var CityName = ObjCurrRow.find('td:eq(' + TblIndx.CityName + ')')[0].getElementsByTagName('select')[0].value;
    var CityName = ObjCurrRow.find('td:eq(' + TblIndx.CityName + ') select option:selected').text()
    var DealerName = ObjCurrRow.find('td:eq(' + TblIndx.AccountDesp + ') select option:selected').text()
    if (CityName !== '' && DealerName!=='') {
        ...
        $('#ddlCity1').prop('disabled', false).trigger('change');
        var StateName = '';
        ObjCurrRow.find('td:eq(' + TblIndx.StateName + ')')[0].getElementsByTagName('input')[0].value = StateName;
    }
...
function AddNewRow()
    ...
    if (convertDateFormat(TodayDate) ==convertDateFormat(dtPlanDate)){
        ...
        VisitType.innerHTML = '<select id="ddlVisitType' + tbItemConsumeRowNo + '" class="form-control form-control-sm box_border" name="ddlVisitType"  autocomplete="off" onchange="getVisitType(\'ddlVisitType' + tbItemConsumeRowNo + '\',\'ddldealerName' + tbItemConsumeRowNo + '\',' + tbItemConsumeRowNo + ');"></select>';
...
function IsVaildFrm(x) {
    ...
    var val = $('#ddlVisitType1 option:selected').text()
...
function getVisitType(elementId, txtdealer, rowNo) {
    ...
    if (val === "New Acquisition") {
       ...
       //$('#ddlCity' + rowNo).prop('disabled', false).trigger('change');
        ...
        //GetCityList('India', 'All');
        $('#ddlCity' + rowNo).val(null).trigger('change');
        ...
        if (ios) {
            ...
            if (userAgent.includes('wv')) {
                $('#ddldealerName' + rowNo).next('.select2-container').show();
                $('#txtdealer' + rowNo).attr("hidden", hidden);
                $('#txtdealer' + rowNo).attr("readonly", readonly);
                $('#txtdealer' + rowNo).val('');
                //$('#' + txtdealer).attr("list", "listdealer");
                //$('#' + txtdealer).attr("readonly", false);
                //$('#' + txtdealer).val('');
...
//        GetAccountMasterDetails('#ddldealerName' + rowNo,rowNo);
//    }
//}
...
function GetAccountMasterDetails(element, rowNo) {
    ...
    var selectedText = $(element).select2('data')[0].text;

    if (selectedText == '') {
        $('#txtState' + rowNo).val('');
        //$('#ddlCity' + rowNo).prop('disabled', true).trigger('change');
        //$('#ddlCity' + rowNo).empty().trigger('change'); 
